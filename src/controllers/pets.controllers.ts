...
export const getPets = (
    ...
    if(adopted) {
        filteredPets = pets.filter((pet: Pet): boolean => pet.adopted === JSON.parse(adopted))
    }
    if(minAge) {
        filteredPets = pets.filter((pet: Pet): boolean => pet.age >= JSON.parse(minAge))
    }
    if(maxAge) {
        filteredPets = pets.filter((pet: Pet): boolean => pet.age <= JSON.parse(maxAge))
    }
